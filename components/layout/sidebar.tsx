...
export default function Sidebar() {
  ...
  return (
    <SidebarShell
      ...
      collapsedWidth="w-20"
      asideClassName="hidden lg:flex overflow-hidden border-r border-gray-200 bg-[#f5f7f6] py-6 shadow-[4px_0_18px_rgba(0,0,0,0.06)]"
      backdrop={false}
    >
      {({ expanded, open, close }) => {
        const handleClickCapture = (event: React.MouseEvent<HTMLElement>) => {
          const isInteractive =
            ...
            Boolean(event.target.closest("[data-sidebar-interactive='true']"));
        ...
        return (
          <div
            className={cn("flex h-full w-full min-h-0 flex-col", expanded ? "px-5" : "px-3")}
          ...
          >
            ...
            <Link
              href="/"
              className={cn("mb-8 flex items-center", expanded ? "gap-3 px-2" : "justify-center")}
            >
