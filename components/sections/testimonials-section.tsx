...
export default function TestimonialsSection() {
    return (
        <section id="testimonials" className="py-20 bg-brand-surface-alt relative overflow-hidden">
            {/* Decorative */}
            <div className="absolute top-10 left-10 w-40 h-40 rounded-full bg-brand-light opacity-5" />
            <div className="absolute bottom-10 right-10 w-56 h-56 rounded-full bg-brand-mid opacity-5" />
            ...
            <div className="max-w-360 mx-auto px-6 lg:px-23 relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {testimonials.map((t) => (
                        <div
                            key={t.name}
                            className="reveal-item bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl border border-brand-surface flex flex-col gap-4 transition-all duration-300 ease-out"
                        >
                            {/* Quote icon */}
                            <Quote size={24} className="text-brand-light opacity-60" />
                            ...
                            {/* Author */}
                            <div className="flex items-center gap-3 pt-2 border-t border-brand-surface">
                                <div
                                ...
                                <div>
                                    <p className="text-brand-dark font-bold text-[14px]">{t.name}</p>
                                    <p className="text-brand-dark/50 text-[12px]">{t.role}</p>
                                </div>
                            </div>
                        </div>
                    ))}
